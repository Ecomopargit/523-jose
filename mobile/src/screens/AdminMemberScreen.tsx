...
import type { MemberProfile, MemberStatus, RootStackParamList } from "../types";
...
export function AdminMemberScreen({ navigation, route }: Props) {
  ...
  useEffect(() => {
    void getMember(route.params.memberId).then(setMember).finally(() => setLoading(false));
  ...
  return (
    <SafeAreaView style={styles.safe}>
      ...
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.moneyActions}><Pressable onPress={() => { setAdjustMode("credit"); setAdjusting(true); }} style={[styles.primary, styles.moneyButton]}><Feather color={colors.white} name="plus-circle" size={16} /><Text style={styles.primaryText}>Liberar valor</Text></Pressable><Pressable onPress={() => { setAdjustMode("debit"); setAdjusting(true); }} style={[styles.withdraw, styles.moneyButton]}><Feather color={colors.green700} name="arrow-up-right" size={16} /><Text style={styles.withdrawText}>Realizar saque</Text></Pressable></View>
        ...
        <View style={styles.actions}>
          <Pressable onPress={() => void setStatus("ativo")} style={[styles.action, styles.activate]}><Feather color={colors.green700} name="check-circle" size={18} /><Text style={styles.activateText}>Ativar conta</Text></Pressable>
          <Pressable onPress={() => Alert.alert("Bloquear associado?", "O usuário não poderá acessar a conta.", [{ text: "Cancelar", style: "cancel" }, { text: "Bloquear", style: "destructive", onPress: () => void setStatus("bloqueado") }])} style={[styles.action, styles.block]}><Feather color={colors.danger} name="slash" size={18} /><Text style={styles.blockText}>Bloquear</Text></Pressable>
        </View>
      </ScrollView>

      <Modal animationType="slide" transparent visible={adjusting} onRequestClose={() => setAdjusting(false)}><View style={styles.overlay}><Pressable accessibilityLabel="Fechar" onPress={() => setAdjusting(false)} style={StyleSheet.absoluteFill} /><View style={styles.sheet}><View style={styles.handle} /><Text style={styles.sheetTitle}>{adjustMode === "credit" ? "Liberar saldo" : "Realizar saque"}</Text><Text style={styles.sheetText}>{adjustMode === "credit" ? `O valor ficará disponível imediatamente para ${member.nome}.` : `O valor será debitado do saldo disponível de ${member.nome}. Saldo atual: ${money(member.saldoDisponivel)}.`}</Text><TextInput autoFocus editable={!savingBalance} keyboardType="decimal-pad" onChangeText={setAmount} placeholder="R$ 0,00" placeholderTextColor={colors.inkFaint} style={styles.amount} value={amount} /><Pressable disabled={savingBalance} onPress={() => void addBalance()} style={styles.primary}>{savingBalance ? <ActivityIndicator color={colors.white} size="small" /> : <Text style={styles.primaryText}>{adjustMode === "credit" ? "Confirmar liberação" : "Confirmar saque"}</Text>}</Pressable></View></View></Modal>
    </SafeAreaView>
  );
}
